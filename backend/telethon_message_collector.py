...
async def create_tables():
    ...
    try:
        ...
        await conn.execute(
            "CREATE INDEX IF NOT EXISTS idx_signal_timestamp ON signal_messages(timestamp);"
        )
        await conn.execute(
            "CREATE INDEX IF NOT EXISTS idx_market_timestamp ON market_messages(timestamp);"
        )
        await conn.execute(
            "CREATE INDEX IF NOT EXISTS idx_signal_pair ON signal_messages(pair);"
        )
    ...
    async def handler(event):
        ...
        try:
            ...
            is_signal = (
                ...
                and any("entry" in line.lower() for line in lines)
                and any("profit" in line.lower() for line in lines)
            ...
            async with buffer_lock:
                if is_signal:
                    ...
                    setup_type = (
                        "LONG"
                        if "LONG" in first_line.upper()
                        else "SHORT"
                        if "SHORT" in first_line.upper()
                        else "UNKNOWN"
                    )
                    ...
                    signal_buffer.append(
                        (
                            ...
                            int(to_decimal_safe(leverage.replace("x", "")))
                            if leverage and leverage != "None"
                            else None,
                            to_decimal_safe(tp1),
                            to_decimal_safe(tp2),
                            to_decimal_safe(tp3),
